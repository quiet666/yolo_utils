...
def run_classification_prediction(checkpoint, data, defect_class, predict_dir_name="predict_v3", confidence_threshold=0.9):
    """封装分类任务的预测代码."""
    print("--- 运行分类预测 ---")
    model = YOLO(checkpoint)  
...
def simple_prediction(checkpoint, data, defect_class, predict_dir_name="predict", confidence_threshold=0.9):
    """预测"""
    model = YOLO(checkpoint)  
    base_predict_path = f"./runs/classify/{predict_dir_name}"
    
    results = model.predict(f"{data}/{defect_class}",
                  save=True,
                  name=f"{predict_dir_name}/{defect_class}"
                 )  # predict on an image
...
def main():
    ...
    # 设置要操作的根目录
    model = "yolo11x-cls"
    data = r"/root/autodl-tmp/ultralytics-main/datas/WX_class/v8/v8-binary_split"
    checkpoint = r"/root/autodl-tmp/ultralytics-main/runs/classify/v8-binary/weights/best.pt" 
    delete_ipynb_checkpoints(data)
    
    # run_classification_training(model, data)    
    run_classification_validation(checkpoint, data)
    # simple_prediction(checkpoint, data, defect_class="test", predict_dir_name="predict_v8_simple")
    # run_classification_prediction(checkpoint, 
    #                               data, 
    #                               defect_class="1_defect", #0_normal  1_cl  2_fz  3_py 4_undefined
    #                               predict_dir_name="predict_v8", 
